...
function initAnimationStuffs() {
  ...
  caption = $(".carousel_caption-text");
  flickity = carousel.data("flickity");
  onSelect = function() {
    $(".carousel-cell").css("background-color", "var(--quarternary-theme-color)");
    $(".carousel-cell").css("border-color", "var(--quarternary-theme-color)");
    $(flickity.selectedCell.element).css("background-color", "#42f4a7");
    $(flickity.selectedCell.element).css("border-color", "#42f4a7");
    caption.text(flickity.selectedElement.children[0].children[0].alt);
  };
  // set selected attributes once, then set every time after
  onSelect();
